...
using BizTweak.Configuration;
using BizTweak.EntityFrameworkCore;
using BizTweak.Identity;
using BizTweak.Web.Chat.SignalR;
using BizTweak.Web.Common;
using Swashbuckle.AspNetCore.Swagger;
using BizTweak.Web.IdentityServer;
using BizTweak.Web.Swagger;
using Stripe;
...
using BizTweak.Configure;
using BizTweak.Schemas;
using BizTweak.Web.HealthCheck;
using Newtonsoft.Json.Serialization;
using Owl.reCAPTCHA;
using HealthChecksUISettings = HealthChecks.UI.Configuration.Settings;
using Microsoft.AspNetCore.Server.Kestrel.Https;
using BizTweak.Web.MultiTenancy;

namespace BizTweak.Web.Startup
{
    public class Startup
    {
        ...
        public IServiceProvider ConfigureServices(IServiceCollection services)
        {
            ...
            return services.AddAbp<BizTweakWebHostModule>(options =>
            {
...
app.Use(async (context, next) =>                {                    await next();                    if (context.Response.StatusCode == 404                        && !Path.HasExtension(context.Request.Path.Value))                    {                        context.Request.Path = "/index.html";                        await next();                    }                });
            ...
            if (BizTweakConsts.PreventNotExistingTenantSubdomains)
            {
            ...
            if (WebConsts.SwaggerUiEnabled)
            {
                ...
                {
                    options.SwaggerEndpoint(_appConfiguration["App:SwaggerEndPoint"], "BizTweak API V1");
                    options.IndexStream = () => Assembly.GetExecutingAssembly()
                        .GetManifestResourceStream("BizTweak.Web.wwwroot.swagger.ui.index.html");
        ...
        private void ConfigureSwagger(IServiceCollection services)
        {
            ...
            {
                options.SwaggerDoc("v1", new OpenApiInfo() { Title = "BizTweak API", Version = "v1" });
                ...
                if (canShowSummaries)
                {
                    ...
                    var applicationXml = $"BizTweak.Application.xml";
                    var applicationXmlPath = Path.Combine(AppContext.BaseDirectory, applicationXml);
                    options.IncludeXmlComments(applicationXmlPath);

                    var webCoreXmlFile = $"BizTweak.Web.Core.xml";
